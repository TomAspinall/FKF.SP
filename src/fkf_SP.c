...
/* No outputs? Returns ahatt in att and Vt in Ptt */
{
	...
	// Develop Identity matrix:
	// int *identity_matrix = (int *) Calloc(m_x_m, int);
    ...
    while(t>-1){
		...
		// Rprintf("N at start: %f\n", N[0]);
		// Rprintf("r at start: %f \n", r[0]);
		...
		if(NAsum==0){
			...
			for(int SP = d-1;SP > -1;SP--){
				...
				// Step 2: tmp_scalar = -F_(t,i)^-1:
				// Confirmed Correct:
				tmp_scalar = - Ftinv[(d*t) + SP];

				// Step 3: L_(t,i) = tmp_scalar * K_(t,i) %*% Z_(t,i) + L_(t,i):
				// Confirmed Correct:
				// F77_NAME(dgemm)(dont_transpose, dont_transpose, 
				// 	&m, &m, &intone, 
				// 	&tmp_scalar,
				...
				F77_NAME(dgemm)(dont_transpose, transpose, 
					...
					Zt_temp, &m,
					&dblzero, tmpmxm, &m FCONE FCONE);
				// F77_NAME(dcopy)(&m, Zt_temp, &intone, tmpZ, &intone);
				// F77_NAME(dgemm)(dont_transpose, transpose, 
				// 	&m, &m, &m, 
				// 	&Ftinv[(d*t) + SP],
				// 	tmpZ, &m,
		...
		else
		{
			...
			{    	
				...
				// Step 2: tmp_scalar = -F_(t,i)^-1:
				tmp_scalar = - Ftinv[(d_reduced*t) + SP];
	...
	// Rprintf("r at end: %f \n", r[0]);
		...
		// print_array(N, m, intone, "N:");
		// print_array(r, m, intone, "r:");

	// The values for r_(t,0) and N_(t,0) are the same as the values for the smoothing quantities of r_(t-1) and N_(t-1) of the standard smoothing equations, respectively.
	// #ifdef DEBUGME
		// print_array(N, m, intone, "N:");
		// print_array(r, m, intone, "r:");
	// #endif
...
/* ---------- --------------- Convert between R and C  -------------- ---------- */
/*********************************************************************************/
SEXP fks_SP(SEXP Tt, SEXP Zt,SEXP yt,SEXP vt, SEXP Kt, SEXP Ftinv, SEXP att, SEXP Ptt)
{
