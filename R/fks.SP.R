...
#'conjunction with expectation-maximization to estimate the parameters of a state space model. This function is called after running \code{\link{fkf.SP}}.
#'
#'@param FKF.SP_obj  An S3-object of class "fkf.SP", returned by \code{\link{fkf.SP}}.
#'
#'@details
#'
#'\code{fks.SP} is typically called after the \code{fkf.SP} function to calculate "smoothed" estimates of state variables and their corresponding variances. Smoothed estimates are used
#'when utilizing expectation-maximization (EM) to efficiently estimate the parameters of a state space model.
#'
#'Model parameter estimation can be numerically expensive, with many unknown parameters requiring many iterations of the Kalman filter/smoother to converge upon the parameter set that meets global optima.
#'Sequential processing is therefore an appealling choice to maximise computational efficiency when performing Kalman filtering/smoothing. Sequential processing is a univariate treatment of a multivariate
#'series, where rather than taking the entire observational vector \mjeqn{y_t}{y[t]} as the items for analysis, instead observations are considered individually. Sequential processing has been
#'empirically shown to grow linearly with respect to the dimensions of \mjeqn{y_t}{y[t]}, rather than exponentially as is the case with the traditional Kalman filter/smoother algorithms (Aspinall et al., 2022, P104).
#'
...
#'
#'Given a state space model has been filtered through the sequential processing Kalman filter algorithm described in \code{\link{fkf.SP}}, the smoother can be reformulated for the univariate series:
#'
...
#'
#'The standard smoothing equations for \mjeqn{\hat{a}_{t}}{ahat[t]} and \mjeqn{V_t}{V[t]} are used:
#'
...
#'\mjeqn{v_{t,i}}{v[t,i]} are scalars. The reduced dimensionality of many of the variables in this formulation compared to traditional Kalman smoothing results in increased computational efficiency.
#'
...
#'   containing the variances of \code{ahatt}, i.e. \code{Vt[,,t]} = \mjeqn{P_{t|n}}{P(t|n)}\cr
#'
#'@references
#'
#'Aspinall, T. W., Harris, G., Gepp, A., Kelly, S., Southam, C., and Vanstone, B. (2022). \emph{The Estimation of Commodity Pricing Models with Applications in Capital Investments}.
#'
