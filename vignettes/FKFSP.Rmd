...
This document provides worked examples of Kalman Filtering through the FKF.SP package and the fkf.SP function. Three examples presented within the FKF package are included here, showcasing the difference in computational speed of the two packages and how to properly structure and provide arguments to both Kalman Filtering functions fkf and fkf.SP. The arguments presented are relatively simplistic Kalman Filtering problems, the relative increase in Filtering computational speed would be more significant for problems with more dimensions in observations and the state vector.
...
Autoregression Moving Average models can be estimated through Kalman Filtering. See also \code{help(makeARIMA)} and \code{help(KalmanRun)}.
...
Parameter estimation is peformed through maximum likelihood estimation. This involves optimizing the log-likelihood returned by the Kalman Filter through the 'optim' function.
```{r}
...
if(SP){
 ans <- fkf.SP(a0 = param$a0, P0 = param$P0, dt = param$dt, ct = param$ct, Tt = param$Tt,
               Zt = param$Zt, HHt = param$HHt, GGt = param$GGt, yt = yt)
 return(-ans)
 }
...
Identical estimated parameters:

```{r}
print(rbind(FKF.SP_estimation$par, FKF_estimation$par))
```

Identical function call count:
...
The vignette of FKF shows how to filter the series with estimated parameter values and develop some plots for analysis purposes.

## Example 2 - Local level model for the Nile's annual flow:

This example presents differences in the computational time of the fkf.SP and fkf functions to the Nile dataset. It also shows the difference in log-likelihood values returned by the two functions that occurs when NA's are within observations.
...
When performing parameter estimation using complete data, the fkf and fkf.SP functions return identical results:
...
Performing on parameter estimation using incomplete data returns identical estimated parameters, but different log-likelihood values:
```{r}
...
for(i in 1:1e4) fkf(a0, P0, dt, ct, Tt, Zt, HHt = matrix(fkf_MLE_complete$par[1]),
                    GGt = matrix(fit.fkf$par[2]), yt = rbind(y_complete))
FKF_runtime = Sys.time() - start
...
##Time comparison - Estimate parameters 100 times:
start = Sys.time()
for(i in 1:10){
...
start = Sys.time()
##When FKF.SP is input poorly specified parameters (ie. log-likelihood = NA) is outputs a warning:
suppressWarnings(
for(i in 1:10){
  fit.fkf.SP <- optim(c(HHt = var(y, na.rm = TRUE) * .5,
                        GGt = var(y, na.rm = TRUE) * .5),
                      fn = function(par, ...)
